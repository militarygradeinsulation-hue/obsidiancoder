...
export const STYLE_FAMILIES: readonly StyleFamilySpec[] = [
  {
    ...
    label: "Futuristic",
    layouts: [
      "split-diagonal-console",
      "orbital-radial",
      "hud-overlay-grid",
      "stacked-viewport-panels",
    ],
    heroes: [
    ...
    shape: [
      "0px radius with 1px cyan hairline borders and clipped 12px corner notches",
      "4px radius, 2px solid borders, no shadows",
    ],
    depth: [
      "layered translucent planes with additive glow, no drop shadows",
      "hard 1px outlines plus radial backlight bloom",
    ],
    palettes: [
    ...
    motion: [
      "120ms linear micro-states, 900ms scroll-linked parallax on depth layers",
      "160ms cubic-bezier(.2,.9,.1,1) with staggered 40ms reveals",
    ],
    interactions: [
      "cursor-reactive lighting on the hero canvas",
      "magnetic primary CTA with 8px pull radius",
      "scroll-scrubbed telemetry counters",
    ],
    techniques: [
      "canvas-2d-particles",
      "css-3d-perspective",
    ...
    mobileRules: [
      "drop the canvas to a static gradient under 640px",
      "single column, 56px section-y, disable parallax",
    ],
  ...
  },
  {
    ...
    shape: [
      "0px radius everywhere, no borders, separation by light only",
      "2px radius, hairline 8%-white rules",
    ],
    depth: [
      "multi-layer parallax with a graded vignette and film grain overlay",
      "spotlight radial masks over near-black",
    ],
    palettes: [
    ...
    motion: [
      "600ms ease-out dissolves, scroll-scrubbed camera pans",
      "400ms cubic-bezier(.16,1,.3,1) with 80ms stagger",
    ],
    interactions: [
      "scroll-scrubbed chapter transitions",
      "hover-revealed caption overlays",
      "parallax depth on pointer move",
    ],
    techniques: [
      "scroll-linked-transform",
    ...
    mobileRules: [
      "replace scrub with discrete IntersectionObserver reveals",
      "cap grain overlay opacity, 72px section-y",
    ],
    forbidden: [...UNIVERSAL_FORBIDDEN],
  },
  {
    ...
    spacing: [
      "8px step, 160px section-y, 48px gap, 1180px container",
      "10px step, 128px section-y, 56px gap",
    ],
    shape: [
      "0px radius, 1px 12%-warm-grey hairlines, generous inner padding 48px",
      "2px radius, no borders, whitespace as separation",
    ],
    depth: [
      "no shadows; separation via whitespace and a single soft image glow",
      "one 0 30px 80px -40px shadow on the plinth only",
    ],
    palettes: [
    ...
    motion: [
      "500ms ease-in-out crossfades, 0px hover lift, opacity-only states",
      "350ms ease with 2px letter-spacing shift on links",
    ],
    interactions: [
      "slow crossfade gallery with keyboard arrows",
      "hover reveals a hairline underline that draws from left",
      "sticky side index",
    ],
    techniques: [
      "layered-svg",
    ...
    mobileRules: [
      "stack to one column at 768px, 80px section-y, keep hairlines",
      "swap crossfade gallery for a swipe rail",
    ],
  ...
  },
  {
    ...
    spacing: [
      "8px step, 88px section-y, 32px column gap, 1240px container",
      "6px step, 72px section-y, 24px gap",
    ],
    shape: [
      "0px radius, 1px rules between stories, no cards",
      "0px radius, 3px top rule on section headers",
    ],
    depth: [
      "flat — hierarchy from type size and rules only",
      "one subtle tint band per alternating section",
    ],
    palettes: [
    ...
    motion: [
      "no entrance animation; 120ms link underline only",
      "180ms ease fade-in on images only",
    ],
    ...
    mobileRules: [
      "collapse to one column at 720px, keep 18px body",
      "rules stay, gutters shrink to 16px",
    ],
  ...
  },
  {
    ...
    spacing: [
      "4px step, 48px section-y, 0px gap (touching blocks)",
      "8px step, 64px section-y, 4px gap",
    ],
    shape: [
      "0px radius, 3px solid black borders, hard offsets",
      "0px radius, 2px borders, 6px hard offset shadow",
    ],
    ...
    motion: [
      "0ms or 80ms step transitions, instant hover inversion",
      "100ms with a 6px hard translate on hover",
    ],
    interactions: [
      "hover inverts the whole block",
      "infinite marquee band",
      "click-to-cycle colourway",
    ],
  ...
  },
  {
    ...
    spacing: [
      "8px step, 120px section-y, 24px gap, 720px measure",
      "8px step, 96px section-y, 32px gap",
    ],
    ...
    interactions: [
      ...
      "inline expand/collapse detail",
      "hover reveals metadata in the margin",
    ],
  ...
  },
  {
    ...
    shape: [
      "24px radius everywhere, 2px solid borders, 4px hard offset",
      "9999px pills for every control, 20px cards",
    ],
    depth: [
      "chunky 6px offset shadows in the accent colour",
      "layered stickers with slight rotations",
    ],
    palettes: [
    ...
    motion: [
      "220ms cubic-bezier(.34,1.56,.64,1) springy hovers, 4px lift",
      "180ms with a 2deg wiggle on hover",
    ],
    interactions: [
      "tilt-on-hover cards",
      "confetti burst on primary action",
      "drag-to-reorder sticker board",
    ],
    techniques: [
      "css-3d-tilt",
      "canvas-2d-particles",
  ...
  },
  {
    ...
    depth: [
      "soft 0 24px 60px -30px shadows tinted with the accent",
      "overlapping translucent layers, no hard edges",
    ],
    palettes: [
    ...
    motion: [
      "400ms ease-in-out, 6px float loops on decorative shapes",
      "300ms with gentle scale 1.02 on hover",
    ],
    interactions: [
      "scroll-morphing SVG path",
      "cursor-following soft light",
      "expanding accordion with curve reveal",
    ],
    techniques: [
      "layered-svg",
      "animated-gradient-mesh",
  ...
  },
  {
    ...
    spacing: [
      "4px step, 32px section-y, 12px gap, 1440px container",
      "4px step, 24px section-y, 8px gap, full-bleed",
    ],
    ...
    interactions: [
      "keyboard row navigation with j/k",
      "column sort + sticky header",
      "inline sparkline tooltips",
    ],
    ...
    mobileRules: [
      "table becomes stacked definition rows under 720px",
...
export function selectDNA(input: {
  ...
  for (const fam of families) {
    for (let i = 0; i < 6; i++) {
      ...
      const recencyBonus =
        input.family === "auto" ? (recencyIdx === -1 ? 0.15 : recencyIdx / 100) : 0;
